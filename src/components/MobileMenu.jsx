                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-6">
                      {accountSections.flatMap(s => s.items).map((item) => (
                        ...
                        >
                          ...
                          <span className="text-xs uppercase tracking-[0.1em] font-medium">{item.name}</span>
                        </Link>
                      ))}
                    </div>
                ...
                <div className="space-y-10">
                  {activeCategory.subMenu.columns.map((column, colIdx) => (
                    <div key={colIdx} className="space-y-4">
                      <h3 className="text-[10px] tracking-[0.2em] uppercase text-stone-400 font-medium">
                        {column.title}
                      </h3>
                      <div className="space-y-3">
                        {column.items?.map((item, itemIdx) => (
                          <Link
                            ...
                            to={item.href}
                            onClick={handleClose}
                            className="block text-sm text-stone-700 hover:text-stone-900 tracking-wide font-light transition-colors"
                          >
                            {item.name}
                          </Link>
                        ))}
