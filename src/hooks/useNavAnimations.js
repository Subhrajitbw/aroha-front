// hooks/useNavAnimations.js
import { useEffect } from "react";
import { gsap } from "gsap";

export const useNavAnimations = (navRef, logoRef, iconsRef) => {
  useEffect(() => {
    const ctx = gsap.context(() => {
      if (navRef.current) {
        gsap.fromTo(navRef.current, { y: -100, opacity: 0 }, { y: 0, opacity: 1, duration: 1.2, ease: "power3.out", delay: 0.2 });
      }
      // Logo will follow navRef animation naturally
      if (logoRef.current) {
        gsap.set(logoRef.current, { opacity: 1, rotation: 0, scale: 1 });
      }
      if (iconsRef.current?.length) {
        // Filter out any null elements in the icons array
        const validIcons = iconsRef.current.filter(Boolean);
        if (validIcons.length) {
          gsap.fromTo(validIcons, { y: 30, opacity: 0, scale: 0.8 }, { y: 0, opacity: 1, scale: 1, duration: 0.8, ease: "back.out(1.7)", stagger: 0.1, delay: 0.8 });
        }
      }
    });

    return () => ctx.revert();
  }, [navRef, logoRef, iconsRef]);
};
